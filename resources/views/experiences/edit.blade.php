...
@section('content')
    <div class="card shadow-sm p-4">
        <h2 class="mb-4">Modifier une expérience</h2>
        ...
        <form action="{{ route('experiences.update', $experience->id) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="city" id="city" class="form-control"
                        value="{{ old('city', $experience->city) }}" required>
                </div>

                <div class="col-md-6 mb-3">
                    <label for="postal_code" class="form-label">Code postal</label>
                    <input type="text" name="postal_code" id="postal_code" class="form-control"
                        value="{{ old('postal_code', $experience->postal_code) }}">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="address" id="address" class="form-control"
                    value="{{ old('address', $experience->address) }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="user_id" id="user_id" class="form-select" required>
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('user_id', $experience->user_id) == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
